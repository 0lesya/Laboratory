...
def v_calculation(file, n):
    count_dict = dict()
    for line in file:
        line = bytes(line, 'utf-8').hex()
        for i in range(len(line)-n):
            if n == 2:
                okt = line[i]+line[i+1] + line[i+2]
            elif n == 3:
                okt = line[i] + line[i + 1] + line[i + 2] + line[i+3]
            else:
                okt = line[i] + line[i + 1] + line[i + 2] + line[i+3] + line[i+3]
...
def calculation_two_substrings(file, n):
    count_dict = dict()
    for line in file:
        line = bytes(line, 'utf-8').hex()
        for i in range(len(line) - n):
...
N = int(input())
calculation_two_substrings(open_file(fileName), N)

conditional_probability(substring_probability(v_calculation(open_file(fileName), N)),
